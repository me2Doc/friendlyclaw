...
Located in `brain/core/agent.py`. This is the primary LLM interface. It handles real-time chat, tool selection, and overall mission strategy. It uses **Recursive Chain-of-Thought** to solve complex problems in a single turn.

### 2. The Swarm (Ghost Workers)
Located in `brain/core/swarm.py`. The Commander can delegate long-running or parallel tasks to this layer.
...
Located in `brain/memory/memory.py`. 
...
Located in `body/`. Powered by **OpenClaw**, this Node.js layer provides the "muscles" (Shell, UI control, Media, etc.).
